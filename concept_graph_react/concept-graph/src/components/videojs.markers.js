  ...
  function updateBreakOverlay(): void {
    ...
    if (
      ...
      if (overlayIndex !== currentMarkerIndex) {
        ...
        if (breakOverlay) {
          let displayHTML = setting.breakOverlay.text(marker);

          // when text length is too long, only take the center part
          if (displayHTML.length > setting.breakOverlay.maxLength) {
            // get the center part
            displayHTML = 
            "... " + 
            displayHTML.substring(
              Math.floor(displayHTML.length / 2) - Math.floor(setting.breakOverlay.maxLength / 2),
              Math.floor(displayHTML.length / 2) + Math.floor(setting.breakOverlay.maxLength / 2)
            ) 
            + " ...";
          }
          breakOverlay.querySelector('.vjs-break-overlay-text').innerHTML = displayHTML;
