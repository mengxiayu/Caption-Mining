...
import * as m from "./videojs.markers.js";
import "./videojs.markers.css";

import testVideo from ".././videos/test_21_min.mp4";

import concept_topics from "../data/concept_timeline/lecture_concept_timeline_CS_241.json";

const lecName = "lecture 25-Q05";
...
export function ConceptTimeline() {
    ...
    const [playerKey, setPlayerKey] = React.useState(lecName);
    ...
    let lecNameList = new Set();
    for (let topicName in concept_topics) {
        for (let name in concept_topics[topicName].mentions) {
            lecNameList.add(name);
        }
    }
            ...
            <div className='flex-row'>
                <div className='flex-col'>
                    <Dropdown onSelect={handleSelect}>
                        ...
                        <Dropdown.Menu>
                            {[...lecNameList].sort(function (a, b) {
                                return (Number(a.match(/(\d+)/g)[0]) - Number((b.match(/(\d+)/g)[0])));
                            }).map(lecName => (
                                <Dropdown.Item key={lecName} eventKey={lecName} >{lecName}</Dropdown.Item>
                            ))}
                        </Dropdown.Menu>
                    </Dropdown>
                </div>
                <div ref={playerRef} className='videoHolder'>
                    <VideoJS key={playerKey} options={videoJsOptions} onReady={handlePlayerReady} lecName={playerKey} />
                </div>
            </div>
        </>
    )
...
export const VideoJS = (props) => {
    ...
    const playerRef = React.useRef(null);
    const { options, onReady, lecName } = props;
    ...
    React.useEffect(() => {
        ...
        if (!playerRef.current) {
            ...
            var topicId = 0;
            for (let topicName in concept_topics) {
                topicId = parseInt(topicName.split(" ")[1], 10);
                let keywords = concept_topics[topicName].keywords;
                for (let name in concept_topics[topicName].mentions) {
                    if (name === lecName) {
                        let timeStamps = concept_topics[topicName].mentions[lecName];
                        for (let i = 0; i < timeStamps.length; i++) {
                            // if (parseInt(timeStamps[i].split(" ")[0], 10) === 0) { continue; }
                            let marker = {
                                time: parseInt(timeStamps[i].split(" ")[0], 10) * 60 + 30,
                                text: "Concepts: <br/>" + keywords.join("; "),
                                overlayText: concept_topics[topicName].captions[lecName][i],
                                class: markerClassList[topicId % markerClassList.length],
                            };
